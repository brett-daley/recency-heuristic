...
def calculate(estimator: str, v, next_v, rewards, terminateds, truncateds, discount):
    ...
    if tokens[0] == 'lambda':  # Uncorrected lambda-returns
        ...
        G += pow(lambd, N - 1) * _calculate_n_step_returns(N, rewards, bootstraps, discount, dones)
        return G
        ...
        G += (1 - total_weight) * _calculate_n_step_returns(n_func(N), rewards, bootstraps, discount, dones)
        return G
